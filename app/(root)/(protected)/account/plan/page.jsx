...
const PricingCard = ({ title, price, description, features, bgColor }) => (
    <div className={`${bgColor} text-stone-700 overflow-hidden`}>
        <div className="bg-brand-lemon-yellow px-4 py-6">
            <h3 className="text-2xl font-bold tracking-widest uppercase">{title}</h3>
        </div>
        <div className="p-6">
            <div className="text-5xl font-bold uppercase tracking-wider mb-4">{price}<span className='text-lg'> SOL/MONTH</span></div>
            <p className="text-xl tracking-wide mb-8">{description}</p>
            {features.map((feature, index) => (
                <PlanFeature key={index} {...feature} />
            ))}
            <button className="px-6 py-3 mt-8 border border-stone-700 rounded-full text-base font-bold tracking-wide">
                Upgrade
            </button>
        </div>
    </div>
...
function PricingPlans() {
    ...
    return (
        <section className="pt-0 pb-8 md:py-16 lg:py-20">
            <h2 className="text-4xl lg:text-6xl font-bold uppercase text-zinc-800 tracking-wider mb-6">
                ACCOUNT PLANS
            </h2>
            <p className="text-xl font-bold tracking-wide text-stone-700 max-w-6xl mb-16">
                We have introduced four plans for users to increase the visibility of their campaigns which allows users to make their fundraiser popular around the globe to achieve their goal sooner.
            </p>
